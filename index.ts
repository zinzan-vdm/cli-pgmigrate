...
async function main(argv: string[]): Promise<number> {
	...
	if (parseErr) {
		...
		console.error('Usage: pgmigrate [--to <version>] [--from <version>] [--source <dir>] [--config <file>] [--force] <database_uri>');
		return 1;
	}
	...
	try {
		...
		if (opts.from) {
			...
			if (state.version === null) {
				...
				console.log(`Fresh database. Starting from version '${fromVersion}' (before first migration).`);
			} else {
				fromVersion = state.version;
				console.log(`Current database version: '${fromVersion}'${state.dirty ? ' [DIRTY]' : ''}`);
			}
		}

		if (sameVersion(fromVersion, opts.to)) {
			console.log(`Database is already at version '${opts.to}'. Nothing to do.`);
			return 0;
		}
		...
		console.error(`Unexpected version ordering: from '${fromVersion}', to '${opts.to}'`);
		return 1;
...
async function readState(
	...
	try {
		...
		return [null, new Error(`Failed to read state: ${err instanceof Error ? err.message : String(err)}`)];
	}
}
