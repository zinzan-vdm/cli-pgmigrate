...
export namespace Track {
	...
	export async function lock(
		...
		try {
			...
			return [null, new Error(`Failed to acquire advisory lock: ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
	...
	export async function unlock(
		...
		try {
			...
			return [null, new Error(`Failed to release advisory lock: ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
	...
	export async function ensureTable(
		...
		try {
			...
			return [null, new Error(`Failed to ensure tracking table '${tableName}': ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
	...
	export async function getState(
		...
		try {
			...
			return [null, new Error(`Failed to read tracking state: ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
	...
	export async function setVersion(
		...
		try {
			...
			return [null, new Error(`Failed to record version '${version}' in tracking log: ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
}