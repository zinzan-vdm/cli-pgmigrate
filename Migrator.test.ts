...
describe('Migrator.listVersions', () => {
	...
	it('should list version directories sorted ascending', async () => {
		...
		await writeFile(join(tmpDir, '20260612T0015-00', 'up.sql'), 'CREATE TABLE test (id int);');
		await writeFile(join(tmpDir, '20260612T0015-01', 'up.sql'), 'ALTER TABLE test ADD COLUMN name text;');
		await writeFile(join(tmpDir, '20260612T0030-00', 'up.sql'), 'CREATE INDEX idx_test_name ON test(name);');
	...
	it('should detect down.sql when present', async () => {
		await mkdir(join(tmpDir, '20260612T0015-00'), { recursive: true });
		await writeFile(join(tmpDir, '20260612T0015-00', 'up.sql'), 'CREATE TABLE test (id int);');
		await writeFile(join(tmpDir, '20260612T0015-00', 'down.sql'), 'DROP TABLE test;');
