...
export namespace Migrator {
	...
	export async function forward(opts: ApplyOpts): Promise<Types.Result<void>> {
		...
		try {
			...
			if (state.dirty) {
				return [null, new Error(`Database is dirty at version '${state.version}'. Use --force to recover after fixing the issue.`)];
			}

			const [versions, listErr] = await listVersions(source);
			if (listErr) return [null, listErr];

			const targets = versions.filter(v => v.version > from && v.version <= to);
			if (targets.length === 0) {
				return [null, new Error(`No versions between '${from}' and '${to}' to apply.`)];
			}

			for (const ver of targets) {
				const [_c, dirtyErr] = await Track.setVersion(sql, tableName, ver.version, true);
				...
				const [_d, cleanErr] = await Track.setVersion(sql, tableName, ver.version, false);
	...
	export async function backward(opts: ApplyOpts): Promise<Types.Result<void>> {
		...
		try {
			...
			if (stateErr) return [null, stateErr];
			if (state.dirty) {
				return [null, new Error(`Database is dirty at version '${state.version}'. Use --force to recover.`)];
			}
			...
			if (targets.length === 0) {
				return [null, new Error(`No versions to roll back between '${from}' and '${to}'.`)];
			}

			for (const ver of targets) {
				const [_g, dirtyErr] = await Track.setVersion(sql, tableName, ver.version, true);
				if (dirtyErr) return [null, dirtyErr];

				if (!ver.downPath) {
					return [null, new Error(`Version '${ver.version}' is missing down.sql — cannot roll back.`)];
				}
				...
				const newVersion = sortedIdx > 0 ? allVersions[sortedIdx - 1]!.version : '';

				const [_h, cleanErr] = await Track.setVersion(sql, tableName, newVersion, false);
				if (cleanErr) return [null, cleanErr];
			}
	 ...
	 */
	export async function listVersions(source: string): Promise<Types.Result<Types.VersionDir[]>> {
		...
		try {
			...
			for (const entry of entries) {
				...
				let hasUp = false;
				let hasDown = false;
				try { hasUp = statSync(upPath).isFile(); } catch { /* ignore */ }
				try { hasDown = statSync(downPath).isFile(); } catch { /* ignore */ }

				if (!hasUp) {
					return [null, new Error(`Version directory '${entry.name}' is missing up.sql.`)];
				}
			...
			return [null, new Error(`Failed to read source directory '${source}': ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
	...
	async function readFileContent(path: string): Promise<Types.Result<string>> {
		try {
			...
			return [null, new Error(`Failed to read '${path}': ${err instanceof Error ? err.message : String(err)}`)];
		}
	}
}