...
describe('Config.load', () => {
	it('should return defaults when file does not exist', async () => {
		const mockDeps: Config.load.Dependencies = {
			...
			parser: {
				parse: (_input: string): Types.Result<unknown> => [{}, undefined],
			},
		};
	...
	it('should use default when config has no tracking section', async () => {
		const mockDeps: Config.load.Dependencies = {
			...
			parser: {
				parse: (_input: string): Types.Result<unknown> => [{ other: true }, undefined],
			},
		};
	...
	it('should return error when YAML parsing fails', async () => {
		const mockDeps: Config.load.Dependencies = {
			...
			parser: {
				parse: (_input: string): Types.Result<unknown> => [null, new Error('Parse error at line 1')],
			},
		};
